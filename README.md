...
- **Image preview**: images render inside the right panel using the
  ...
  when `TERM=xterm-kitty` (kitty, wezterm). Other terminals show metadata plus
  a hint. PNG/JPEG/GIF stream as-is; BMP/WebP/TIFF/ICO are re-encoded to PNG.
...
| `state`     | Application state: cwd, parent, entries, selection, dialogs, clipboard, navigation |
| `input`     | Key events → state transitions                          |
| `dialogs`   | Dialog business logic (submitting mutations, confirms)  |
| `preview`   | Text/binary preview content                             |
| `img`       | Kitty graphics protocol: image placement, fitting, base64 chunking, delete |
| `ui`        | All Ratatui rendering (panels, help, dialogs, status)    |
